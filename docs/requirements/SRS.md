...
| FDIR-011 | The system shall detect a rail overcurrent when any switchable rail's draw exceeds 0.25 A (nominal ceiling) continuously for at least 500 ms, and shall raise it as a fault carrying recovery authority but NOT SAFE-mode authority. Detection shall occur on current, before any bus-voltage threshold is breached. *(ceiling and window targets, TBD against real hardware)* | Simulated | Test |
| FDIR-012 | The system shall detect a single device returning physically impossible readings across 5 consecutive samples while its data bus remains healthy, and shall diagnose it as a device fault rather than a data-path fault. The flag shall carry neither SAFE-mode nor recovery authority. | Simulated | Test |
| FDIR-013 | The system shall capture a bus-voltage reference from at least 20 clean samples at commissioning, persist it across resets, and raise a drift fault when the measured value departs from that reference by more than 0.25 V for at least 2 s. The reference shall NOT be recaptured on reboot. | Simulated | Test |
| FDIR-014 | The system shall provide at least three pre-validated capability sets and shall autonomously select a more degraded set when deterministic evidence supports it, never on advisory evidence alone. No set shall remove power from the flight computer or, at the most degraded level, the radio. Restoration of capability shall require operator command. *(power budgets are DECLARED, not measured — see fdir/degraded.py)* | Simulated | Test |
