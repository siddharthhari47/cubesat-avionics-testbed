...
| FDIR-002 | Sensor non-response (I2C/SPI timeout) shall be detected within 200 ms and raise the corresponding fault flag. *(target, TBD)* | Test |
| FDIR-003 | The system shall detect undervoltage at two severity thresholds (warning, critical) and enter SAFE mode on critical undervoltage. | Test |
